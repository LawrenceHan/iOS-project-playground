...
-- Requires super-user to run below:
...
-- audit table unless you REALLY need them. The json GIST indexes are
...
BEGIN
    ...
    IF (TG_OP = 'UPDATE' AND TG_LEVEL = 'ROW') THEN
        h_old = hstore(OLD.*);
        h_new = hstore(NEW.*);
        audit_row.row_data = row_to_json(OLD.*);
        h_modified = (h_new - h_old) - excluded_cols;
        audit_row.changed_fields =  hstore_to_json_loose(h_modified);
        IF h_modified = '' THEN
    ...
    ELSIF (TG_OP = 'DELETE' AND TG_LEVEL = 'ROW') THEN
        h_old = hstore(OLD.*);
        audit_row.row_data = hstore_to_json_loose(h_old - excluded_cols);
    ELSIF (TG_OP = 'INSERT' AND TG_LEVEL = 'ROW') THEN
        h_new = hstore(NEW.*);
        audit_row.row_data = hstore_to_json_loose(h_new - excluded_cols);
