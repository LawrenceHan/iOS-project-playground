class CreateCommonSearches5 < ActiveRecord::Migration
  def up
    ...
    filename = File.expand_path('../views/common_searches_5.sql', File.dirname(__FILE__))
    # remove UTF-8 BOM
  ...
  def down
    ...
    filename = File.expand_path('../views/common_searches_4.sql', File.dirname(__FILE__))
    # remove UTF-8 BOM
    execute IO.read(filename).gsub!("\xEF\xBB\xBF".force_encoding("UTF-8"), '')
