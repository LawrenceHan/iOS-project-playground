module V1::Reviews
  describe MedicalExperiences::API do
    describe 'GET /v1/reviews/medical_experiences' do
      ...
      it 'gets medical experiences list' do
        get '/v1/reviews/medical_experiences'
    ...
    describe 'GET /v1/reviews/medical_experiences/:id' do
      let(:user) { create :user }
      before do
        login_as user
      ...
      it 'gets a medical_experience' do
        get "/v1/reviews/medical_experiences/#{@medical_experience.id}"
        expect(response.status).to eq 200
        parsed_body = JSON.parse(response.body)
      ...
      it 'gets error if medical experience not found' do
        get '/v1/reviews/medical_experiences/0'
        expect(response.status).to eq 400
        expect(response.body).to eq JSON.generate(error: ["Can not found Medical experience with ID=0"])
      end
    end

    describe 'POST /v1/reviews/medical_experiences' do
      let(:user) { create :user }
      before { login_as user }

      it 'creates a medical experience' do
        expect {
          post '/v1/reviews/medical_experiences', medical_experience: {network_visible: true}
        }.to change { user.medical_experiences.count }.by(1)
        expect(response.status).to eq 201
      end
    end

    describe 'PUT /v1/reviews/medical_experiences/:id' do
      let(:user) { create :user }
      before do
        login_as user
      ...
      it 'updates a medical experience' do
        put "/v1/reviews/medical_experiences/#{@medical_experience.id}", medical_experience: {network_visible: false}
      ...
      it 'fails to update a medical_experience if not found' do
        put '/v1/reviews/medical_experiences/0', medical_experience: {network_visible: false}
        expect(response.status).to eq 422
      end
    end

    describe 'POST /v1/reviews/medical_experiences/calculation_for_local' do
      it 'gets a new medical experience' do
        post '/v1/reviews/medical_experiences/calculation_for_local', medical_experience: {network_visible: true}
        expect(response.status).to eq 201
        parsed_body = JSON.parse(response.body)
