        ...
        post ':type' do
          ...
          if review.errors.empty?
            say_succeed review.as_json(only: [:id, :medical_experience_id, :created_at, :note],
                                       methods: [:reviewable_name, :user_id])
          else
            say_failed review
          end
        end
        ...
        get ':id', requirements: { id: /[0-9]*/ } do
          if review = Review.includes(:profile, :questions).find_by(id: params[:id])
            ...
            json_config = {
              ...
              include: {answers:
                        { only: [:rating],
                          methods: [:human_waiting_time, :question_type, :question_content]}},
              methods: [:already_marked_as_helpful, :username, :small_avatar, :reviewable_name, :reviewable_id, :user_id]
            }
            case review.type
            when 'HospitalReview'
              json_config[:methods].concat([:hospital_h_class, :hospital_address, :hospital_avg_rating,
                                            :hospital_human_avg_waiting_time, :human_conditions, :human_symptoms])
